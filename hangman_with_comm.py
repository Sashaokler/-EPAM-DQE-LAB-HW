...
def antycheat(user_input: str, last_user_input: str, initial_len_of_input: int, user_approved_letter: str) -> \
typing.Tuple[str, int]:
    """
        ...
        :param user_agreed_letter: буква в слове, наличие которой согласовании игроком.
    """
    ...
    # перевірка щоб підтверджена буква була використана
    has_not_agreed_letter: typing.List[bool] = [corrected_input[i] != user_approved_letter for i in range(
        initial_len_of_input)]  # false - в патерні гравця є підтверджена буква.

    agreed_letter_not_used: bool = all(
        has_not_agreed_letter)  # якщо false - патерн валідний.
    while agreed_letter_not_used:
        print("Что-то тут не так.")
        print("Вы не использовали выбранной буквы.")
        print(f"А именно {user_approved_letter}.")
        corrected_input = input("Давай, соберись. Попробуй снова   ").lower()

        has_not_agreed_letter: typing.List[bool] = [corrected_input[i] != user_approved_letter for i in
                                                    range(initial_len_of_input)]

        agreed_letter_not_used: bool = all(has_not_agreed_letter)
...
def play_game():
    ...
    guesses: str = ""
    current_word: str = ""
    user_approved_letter: str = "" # доданий новий параметр
    len_of_word: int = -1
    ...
    while is_playing:
        if was_correct:
            ...
            current_word, len_of_word = antycheat(current_word, last_word, len_of_word, user_approved_letter)
        ...
        ##
        user_agreed_letter = likeliest_letter[0]
        ##
        print("")
